...
def get_podcast_data_for_user(username, podcast_id):
    ...
    if userData != None:
        likes_data = get_likes_data(userData, podcast_id)
        comments_data = get_comments_data(userData, podcast_id)
        return {'likes': likes_data, 'comments': comments_data}
    return None
...
def get_user_data(username):
    return UserData.objects.filter(username=username).first()
