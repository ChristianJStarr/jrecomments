...
function updatePodcastView(){
    ...
    $.each(podcasts, function (index, podcast){
        ...
        container.append(podcastTemplate.format(podcast.id, podcast.name, podcast.duration));
    });
}
...
function turnOn(podcast){
    var podcastId = podcast.attr('id').split('-')[1];
    var podcastData = podcasts[podcast_id];
    ...
    content.append(dataBar.format(podcastData.date, podcastData.duration, podcastData.totalComments));
