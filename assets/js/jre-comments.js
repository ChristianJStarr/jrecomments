...
const max_podcasts_initial = 20; // Amount of podcasts initially loaded
const max_podcasts_loadin = 20; // Amount of podcasts that are loaded when scroll end
const max_comments_intial = 20; // Amount of comments initially loaded
let can_load_in_podcasts, can_load_in_comments = true; // For scroll end loading

let opened = 0; // Podcast ID - Podcast Currently Opened - None if 0
let lastOpened = 0; // Podcast ID - Last Opened Podcast - ^^
let podcasts = {}; // PK: PodcastID, array of podcast datas
let loaded_podcasts = []; // Array of loaded podcast elements

let userdatas = {};
let liked = []; // Authenticated Users Liked Podcasts (IDS)
let disliked = []; // Authenticated Users Disliked Podcasts (IDS)
...
let objectsLoaded = 0;
let lastObjIndex = 0;
let shouldClearStrength = 0;
let clearAwayObjects = 100;
let clearing = false;
let currentSearch = '';

let playing = false;
let playingId = 0;
const playerSrc = 'https://open.spotify.com/embed/episode/{0}?utm_source=generator&theme=0';
let currentPlayingGlowInterval;
